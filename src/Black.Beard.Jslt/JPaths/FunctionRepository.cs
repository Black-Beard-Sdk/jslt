...
public static class FunctionRepository
{
        ...
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] Type functionType)
    {
        ...
        FindEvaluationMethods(function, functionType, typeof(JToken));
    }
        ...
        Type returnType)
    {
        ...
        var parameters = method.GetParameters()
            .Select(x =>
            {
                if (x.ParameterType == typeof(JToken)) return FunctionType.Value;
        ...
        if (parameters.Any(x => x == null))
            throw new ArgumentException($"'Evaluate' method parameters may only be of types '{typeof(JToken)}', '{typeof(bool)}', or '{typeof(NodeList)}'.");
