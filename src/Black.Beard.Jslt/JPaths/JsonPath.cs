...
public class JsonPath
{
    ...
    /// </summary>
    // public static JsonPath Root { get; } = new(PathScope.Global, new[]);
    public static JsonPath Root { get; } = new(PathScope.Global, new List<PathSegment>());
    ...
    public PathResult Evaluate(JToken? root)
    {

        IEnumerable<Node> currentMatches = new List<Node>() { new(root, Root) };
    ...
    public bool TryToMatch(JsonPath evaluatedPath, bool full = true)
    {
        ...
        for (var i = 0; i < this.Segments.Length; i++)
        {
            ...
            if (evaluatedPath.Segments.Length > i)
            {
                var e = evaluatedPath.Segments[i];
                if (!s.TryToMatch(e))
                    return false;
            }
            else if (full)
    ...
    /// <summary>
    /// Implicit conversion from <see cref="JsonPath"/> to <see cref="string"/>.
    /// </summary>
    /// <param name="path"></param>
    public static implicit operator string(JsonPath path)
    {
        return path.ToString();
    }

    /// <summary>
    /// Implicit conversion from <see cref="string"/> to <see cref="JsonPath"/>.
    /// </summary>
    /// <param name="path"></param>
    public static implicit operator JsonPath(string path)
    {
        return Parse(path);
    }
